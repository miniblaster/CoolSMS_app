...
const CreatePasswordPage = () => {
  ...
  useEffect(() => {
    if (!auth.email) router.replace('/auth/launch')
  }, [auth.email, router])
  ...
  const onSubmit = data => {
    ...
    auth.activate({ email, password, nickName: username, uuid: auth.uuid }, err => {
      setError('email', {
        type: 'manual',
      ...
      <RightWrapper sx={skin === 'bordered' && !hidden ? { borderLeft: `1px solid ${theme.palette.divider}` } : {}}>
        ...
        >
          <BoxWrapper>
            ...
            <Box sx={{ mb: 6 }}>
              <TypographyStyled variant='h5'>Create New Password</TypographyStyled>
            </Box>
